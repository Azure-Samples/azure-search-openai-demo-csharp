...
public sealed class LookupSkill
{
    ...
    public async Task<string> ExecAsync(string lookupQuery, SKContext context) => lookupQuery is string query
            ? await _searchClient.LookupAsync(query, _requestOverrides)
